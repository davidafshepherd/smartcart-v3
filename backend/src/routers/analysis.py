...
async def sam3_automated(
    ...
    foods_dict = _fetch_foods_dict(request.foods, db)
    ...
    for food_id in request.foods:
        ...
        try:
            ...
            raise HTTPException(
                ...
                detail=f"SAM3 inference failed for '{food.short_name}': {e}",
            )
...
async def sam3_assisted_text(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"SAM3 inference failed on the before image: {e}",
        )
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"SAM3 inference failed on the after image: {e}",
        )
...
async def compute_nutrition(
    ...
    # Calculate the consumed volumes of each food in the meal.
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Failed to compute food volumes: {e}.",
        )
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Failed to compute nutrition values: {e}.",
        )
...
def _calculate_masses(
    ...
    food_ids = [volume.food_id for volume in volumes]
    ...
    for volume in volumes:
        ...
        food = foods_dict[volume.food_id]
